...
- `.npmrc`: `ignore-scripts=true`（依存の postinstall 等を遮断）/ `save-exact=true`（exact ピン）/ `min-release-age=7`（公開7日未満のバージョンを除外。内部で `before` 日付に変換される）/ `engine-strict=true`。install スクリプトが必要な依存は個別に許可する。
- 依存導入後は CI で `npm ci`（lockfile 厳格）＋ `npm audit` を回す（package.json 作成時に追加）。
- Dependabot（`.github/dependabot.yml`）で npm・github-actions を更新。`cooldown.default-days: 7` で新規リリースを寝かせる。
- GitHub Actions は commit SHA で固定する（タグ乗っ取り対策。例: `actions/checkout@<sha> # v4.3.1`）。
