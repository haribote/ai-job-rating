...
**Phase 1（MVP）— DoD 全項目達成**。D1/R2 永続化・スコア順ランキング・設定UI（重み変更で即再ランキング・AI非再実行）・一覧/SPA/認証取得・フォーク用整備（#29）を実装済みで live 検証済。抽出機構の方針と既定モデルも確定（#65/#72: 差し替え可能アダプタ + コード側 検証/修復/正規化、既定 = `@cf/meta/llama-3.3-70b-instruct-fp8-fast` / json_mode）。残 open は任意・非ブロックの #88（Post-DoD: モデル非依存の構造的抽出エラー是正）のみ。進行したら更新する。
