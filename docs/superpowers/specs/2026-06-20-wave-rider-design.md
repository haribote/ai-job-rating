...
### 機構限定プローブ結果（2026-06-20 実施）

#10 が #6（雛形）未着手で検証基盤を持たないため、実 issue でなく**捨てブランチによる機構限定プローブ**で 3 未確認点を先行確定した（worktree 分離・background）。

| 未確認点 | 結果 |
|---|---|
| 1. subagent の skill 呼び出し | **可**。Agent サブエージェントは Skill ツールを保持し、`commit`・`pr`(pr-lifecycle) をロードできた。→ **subagent から `/pr`・`/code-review` を直接利用可能**。inline `gh` フォールバックは不要（防御的に残すのみ）。 |
| 2. worktree 分離＋background 通知 | **可**。worktree は `locked` で隔離、node_modules は親 repo への symlink 共有（CLAUDE.md 方針通り）。完了通知も orchestrator に正常到達。 |
| 3. subagent の push 権限 | **可・無確認**。feature ブランチ push は権限プロンプトなしで成功。→ **`main` 誤 push 防止は orchestrator 側のブランチ運用に依存**（後述の運用上の含意を必須化）。 |

**運用上の含意（必須）**: push が無確認で通るため、dev subagent コントラクトに「自分の feature ブランチ以外へ push しない／`main` へ push しない」を明示し、orchestrator は subagent 生成ブランチ名を制御する。プローブの捨てブランチ（worktree・ローカル/リモート）は実施後に削除済み。
...
- ~~spike 1〜3 の挙動（skill 呼び出し可否・background 通知・push 権限）~~ → **2026-06-20 機構限定プローブで全て確定**（上記「機構限定プローブ結果」参照）。subagent は `/pr`・`/code-review` を直接呼べ、worktree 分離・background 通知・feature ブランチ push がいずれも動作。残課題は「push 無確認」に対する orchestrator 側のブランチ運用ガードのみ。
